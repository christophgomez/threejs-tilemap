...
export const SpotifyPlayer = ({
  ...
  const queryState = (state?: PlayerState | null) => {
    ...
    if (playerRef.current) {
      ...
      const onGotState = (state: PlayerState | null) => {
        // console.log("got current state", state);
        handleStateChange(
          state?.paused ?? true,
          state?.track_window?.current_track?.name ?? undefined,
          state?.track_window?.current_track?.artists[0]?.name ?? undefined,
          state?.track_window?.current_track?.uri ?? undefined,
          state?.duration ?? 0,
          state?.position ?? 0
        );
  ...
  const handleResize = () => {
    if (controlsRef.current) {
      const elementWidth = controlsRef.current?.offsetWidth;
      let x = defaultPositionRef.current.x;
      const y = defaultPositionRef.current.y;

      // If the element goes off the right edge of the window
      if (x + elementWidth > window.innerWidth) {
        x = window.innerWidth - elementWidth; // Align right edge of the element with the right edge of the window
      }

      // If the element goes off the left edge of the window
      if (x < 0) {
        x = 0; // Align left edge of the element with the left edge of the window
      }

      setDefaultPosition({ x, y: y });
    }
  };
  ...
  useEffect(() => {
    ...
    if (controlsRef.current) {
      ...
      const x = (window.innerWidth - elementWidth) / 2;
      const y = defaultPositionRef.current.y;
      setDefaultPosition({ x, y: y });
    }
  }, [controlsRef.current]);
